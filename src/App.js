...
function generateMissionsForToday() {
  ...
  if (dayOfWeek >= 1 && dayOfWeek <= 5) {
    ...
    missions.push({ id: 1004, name: "Fregar platos y limpiar la cocina", xp: 10 });
  }
...
// Mensajes emergentes para atributos (ejemplo para "disciplina" y "organizacion"; agregar para los demás atributos según se requiera)
...
const GameProviderWithPopup = ({ children }) => {
  ...
  const [missionsDate, setMissionsDate] = useState(() => loadProgress("missionsDate", ""));
  ...
  // Cada minuto se revisa si cambió el día para procesar penalizaciones y regenerar misiones.
  useEffect(() => {
    const interval = setInterval(() => {
      ...
      if (missionsDate !== todayStr) {
        ...
        missions.forEach(mission => {
          ...
          // Selecciona aleatoriamente un mensaje de castigo
          const punishmentText = castigoMessages[Math.floor(Math.random() * castigoMessages.length)];
        ...
        if (!popup) {
          if (newLevel > level) {
            ...
            const msg = levelDownMessages[Math.floor(Math.random() * levelDownMessages.length)];
            setPopup(msg);
          }
        }
        setXp(newXp);
  ...
  }, [missionsDate, missions, level, xp, attributes, popup]);
  ...
  }, [level, xp, points, attributes, punishments, missions, completedMissions, theme, missionsDate]);
  ...
  // Función para reiniciar todos los datos, incluyendo la regeneración de las misiones diarias
  const resetProgress = () => {
    const todayStr = getTodayStr();
    const newMissions = generateMissionsForToday();
    setLevel(1);
    ...
    saveProgress("missions", newMissions);
    saveProgress("missionsDate", todayStr);
    setPopup("¡Todos los datos han sido reiniciados! Prepárate para comenzar de nuevo.");
  };
      ...
      <div className={`dashboard ${theme}`}>
        <div className="stats">
          ...
          <div className="progress-container">
            <div className="progress-bar" style={{ width: `${(xp / xpToNextLevel) * 100}%` }}></div>
          </div>
    ...
    <div className="page">
      ...
      <div className={`attributes ${theme}`}>
        <p>
          ...
          <button onClick={() => upgradeAttribute("disciplina")} disabled={points === 0}>+</button>
        </p>
        <p>
          Organización: {attributes.organizacion}{" "}
          <button onClick={() => upgradeAttribute("organizacion")} disabled={points === 0}>+</button>
        </p>
        <p>
          Productividad: {attributes.productividad}{" "}
          <button onClick={() => upgradeAttribute("productividad")} disabled={points === 0}>+</button>
        </p>
        <p>
          Gestión del Tiempo: {attributes.gestionTiempo}{" "}
          <button onClick={() => upgradeAttribute("gestionTiempo")} disabled={points === 0}>+</button>
        </p>
        <p>
          Habilidad Técnica: {attributes.habilidadTecnica}{" "}
          <button onClick={() => upgradeAttribute("habilidadTecnica")} disabled={points === 0}>+</button>
        </p>
        <p>
          Salud Física: {attributes.saludFisica}{" "}
          <button onClick={() => upgradeAttribute("saludFisica")} disabled={points === 0}>+</button>
        </p>
        <p>
          Resiliencia: {attributes.resiliencia}{" "}
          <button onClick={() => upgradeAttribute("resiliencia")} disabled={points === 0}>+</button>
        </p>
        <p>
          Bienestar: {attributes.bienestar}{" "}
          <button onClick={() => upgradeAttribute("bienestar")} disabled={points === 0}>+</button>
        </p>
        <p>
          Creatividad: {attributes.creatividad}{" "}
          <button onClick={() => upgradeAttribute("creatividad")} disabled={points === 0}>+</button>
        </p>
        <p>
          Lazos Familiares: {attributes.lazosFamiliares}{" "}
          <button onClick={() => upgradeAttribute("lazosFamiliares")} disabled={points === 0}>+</button>
        </p>
      </div>
    </div>
  );
  ...
  return (
    <div className="page">
      ...
      {punishments.length > 0 ? (
        ...
        <p>No hay castigos registrados.</p>
      )}
    </div>
  );
};
...
const ResetData = () => {
  ...
  return (
    <div className="page">
      ...
      <p>Esta acción borrará y reiniciará todos tus datos guardados, incluyendo las misiones diarias.</p>
      <button onClick={resetProgress}>Reiniciar Datos</button>
    </div>
  );
};
